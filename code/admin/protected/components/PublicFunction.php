...
class PublicFunction{
	 ...
	 * ActiveRecord对象转换为数组
	 */
	public static function getARAttr($record){
		return $record->attributes;
	}
	public static function ARtoArray($activerecord){
		$array = array();
	...
	function Pinyin($_String, $_Code='utf-8')
	{
			...
			if (preg_match("/^\d*$/",substr($_String, 0, 1)))
	        {
	        	$_P = substr($_String, 0, 1);
	        	return strtolower($_NumArray[$_P]);
	        }
	        ...
	        return strtolower($_Res);//preg_replace("/[^a-z0-9]*/", '', $_Res);
	}
	
	private function _Pinyin($_Num, $_Data)
	{
			if($_Num==-9797){ return "qian";}
	...
	public static function str_Empty($array,$key){
		$string = '';
		if (is_array($array) && !empty($array)){
			if (isset($array[$key])){
				$string = empty($array[$key])?'':$array[$key];
			}
		}else{
			$string = '';
		}
